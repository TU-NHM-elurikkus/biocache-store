...
class IdentificationQualifierProcessor extends Processor {
    ...
    def processIdentificationQualifier(rawIdentificationQualifier:String) : (String, ArrayBuffer[QualityAssertion]) = {
        ...
        logger.debug(s"Processing raw.identification.identificationQualifier: ${stringVal}")

        if (StringUtils.isNotBlank(stringVal)) {
            stringVal.trim.replaceAll("\\s+", " ") toLowerCase match {
                case UncertainKeywordRegex (keyword) => logger.debug(s"Identification Qualifier '${stringVal}' is mapped to ${Uncertain}"); translatedIdQualifier = Uncertain
                case CertainKeywordRegex (keyword) => logger.debug(s"Identification Qualifier '${stringVal}' is mapped to ${Certain}"); translatedIdQualifier = Certain
                case _ => logger.debug(s"Identification Qualifier '${stringVal}' is mapped to ${NotRecognised}"); translatedIdQualifier = NotRecognised
            }
        } else {
        ...
        getSource(filePath).getLines.foreach({ line =>
            ...
            if (StringUtils.isNotBlank(line)) {
                line.trim toLowerCase match {
                    case CertainHeader => switch = CertainHeader; logger.debug(s"Encoutered ${CertainHeader}")
                    case UncertainHeader => switch = UncertainHeader; logger.debug(s"Encoutered ${UncertainHeader}")
                    case _ => if (switch == CertainHeader) {
                        if (certainList != "") {
                            certainList = certainList.concat("|" + regex(line))
                        } else {
                            certainList = certainList.concat(regex(line))
                        }
                        logger.debug(s"Adding to certain List: ${certainList}")
                    } else if (switch == UncertainHeader) {
                        if (uncertainList != "") {
                            uncertainList = uncertainList.concat("|" + regex(line))
                        } else {
                            uncertainList = uncertainList.concat(regex(line))
                        }
                        logger.debug(s"Adding to Uncertain List: ${uncertainList}")
                    }
                }
            }
        })

        logger.debug(s"Printing certain List from IdentifcationQualifier.txt : ${certainList}")
        logger.debug(s"Printing uncertain List from IdentifcationQualifier.txt : ${uncertainList}")
    ...
    def getSource(filePath: String): scala.io.Source = {
        ...
        if (overrideFile.exists) {
            ...
            logger.debug(s"Reading identificationQualifier file: ${overrideFile.getAbsolutePath}")
            scala.io.Source.fromFile(overrideFile, "utf-8")
        } else {
            // else use the file shipped with jar
            logger.debug(s"Reading internal identificationQualifier file: ${filePath}")
